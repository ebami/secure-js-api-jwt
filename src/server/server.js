...
app.get("/users", verifyToken, (req, res) => {
  ...
  } else res.status(403).send({ message: "Not authorised to view users" });
});
...
app.get("/favorite", verifyToken, (req, res) => {
  getFavBooksForUser(req.cookies.token).then((books) => {
    generateToken(req.cookies.token, null).then((token) => {
      res.cookie("token", token, { httpOnly: true });
...
app.post("/book", verifyToken, (req, res) => {
  if (!req.body.name || !req.body.author) {
    ...
    } else res.status(403).send({ message: "Not authorised to add book" });
  }
});
